...
float color_shadow[3]     = {0.1f, 0.1f, 0.1f};
                                       ...
                                       bool                           is_trailer)
{
    ...
    // By default we use z-offset 0.05
    double zOffset = 0.05 + 0.05 * elev;

    // If not type moving, we set it to 0.01
...
{
    const unsigned int FAN_SURFACES = 4;
    const double       LENGTH       = bb_x / 2.0;
    const double       WIDTH        = bb_y / 2.0;

    const double extrusion = std::min(SHADOW_MAX_EXTRUSION, 0.1 * ((bb_x + bb_y + bb_z) / 3.0));
    ...
    for (int i = 0; i < 4; i++)
    {
        (*colors)[i].set(color_shadow[0], color_shadow[1], color_shadow[2], 0.5f);
    }
